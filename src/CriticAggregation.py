...
class WeightedByTDErrorWeighing(CriticAggregation):
    ...
    def buildLayer(self):
        ...
        self.q_critic =  tf.reduce_sum(self.weights * (self.q_critic_num / self.q_critic_den))
...
class WeightedByTDErrorWeighingMax(CriticAggregation):

    def __init__(self, sess, qin, td, num_ensemble):
        super(WeightedByTDErrorWeighingMax, self).__init__()
        self._session = sess
        self._q_in = qin
        self._td = td
        self._lr_critic = 0.0001
...
class WeightedByTDErrorNormByExpV2(CriticAggregation):

    def __init__(self, sess, qin, td, num_ensemble):
        super(WeightedByTDErrorNormByExpV2, self).__init__()
        self._session = sess
        self._q_in = qin
        self._td = td
        self._lr_critic = 0.0001
    ...
    def buildLayer(self):
        print('CriticAggregation::WeightedByTDErrorNormByExpV2_buildLayer')

        weights_raw = tf.get_variable(name='weights_raw', dtype=tf.float32,
                                           initializer=np.zeros(self._num_ensemble, np.float32))
        self.weights = tf.nn.softmax(weights_raw)
        self.q_critic = tf.reduce_sum((self._q_in * self.weights))
        qs_loss = tf.reduce_sum(-2*tf.exp(-0.1*(self._td ** 2) + 1e-10)*self.weights)
        self.qs_update = tf.train.AdamOptimizer(self._lr_critic).minimize(qs_loss, name='qs_update')
...
class WeightedByTDErrorNormByExp(CriticAggregation):
    ...
    def __init__(self, sess, qin, td, num_ensemble):
        super(WeightedByTDErrorNormByExp, self).__init__()
        self._session = sess
        self._q_in = qin
        self._td = td
        self._lr_critic = 0.0001
    ...
    def buildLayer(self):
        print('CriticAggregation::WeightedByTDErrorNormByExp_buildLayer')
...
class WeightedByTDErrorAndTailV2(CriticAggregation):

    def __init__(self, sess, qin, td, num_ensemble):
        super(WeightedByTDErrorAndTailV2, self).__init__()
        self._session = sess
        self._q_in = qin
        self._td = td
        self._lr_critic = 0.0001
    ...
    def buildLayer(self):
        ...
        self.q_critic = tf.reduce_sum((self._q_in * self.weights))
        td2td = (self._td ** 2)
        qs_loss = tf.reduce_sum( td2td * self.weights - 4*tf.exp(-0.1*td2td + 1e-10)*self.weights )
        self.qs_update = tf.train.AdamOptimizer(self._lr_critic).minimize(qs_loss, name='qs_update')
...
class WeightedByTDError1E3TarrgetEntropy(CriticAggregation):

    def __init__(self, sess, qin, td, num_ensemble):
        super(WeightedByTDError1E3TarrgetEntropy, self).__init__()
        self._session = sess
        self._q_in = qin
        self._td = td
        self._lr_critic = 0.0001
    ...
    def buildLayer(self):
        ...
        self.qs_loss = tf.reduce_sum( ((self._td ** 2) * self.weights) + (1e3*(0.1 - tf.reduce_sum(-self.weights*tf.math.log(1e-10+self.weights)) ))** 2 ) #-entropy (trying to maximize entropy)
        self.qs_update = tf.train.AdamOptimizer(self._lr_critic).minimize(self.qs_loss, name='qs_update')
...
class WeightedByTDError1E4TarrgetLog2Entropy(CriticAggregation):

    def __init__(self, sess, qin, td, num_ensemble):
        super(WeightedByTDError1E4TarrgetLog2Entropy, self).__init__()
        self._session = sess
        self._q_in = qin
        self._td = td
        self._lr_critic = 0.0001
    ...
    def buildLayer(self):
        ...
        self.qs_loss = tf.reduce_sum( ((self._td ** 2) * self.weights) + (1e4*(0.1 - tf.reduce_sum(-self.weights*tf.math.log(1e-10+self.weights)/tf.math.log(2.0)) ))** 2 ) #-entropy (trying to maximize entropy)
        self.qs_update = tf.train.AdamOptimizer(self._lr_critic).minimize(self.qs_loss, name='qs_update')
...
class WeightedByTDError1E4TarrgetEntropy(CriticAggregation):

    def __init__(self, sess, qin, td, num_ensemble):
        super(WeightedByTDError1E4TarrgetEntropy, self).__init__()
        self._session = sess
        self._q_in = qin
        self._td = td
        self._lr_critic = 0.0001
    ...
    def buildLayer(self):
        ...
        self.qs_loss = tf.reduce_sum( ((self._td ** 2) * self.weights) + (1e4*(0.1 - tf.reduce_sum(-self.weights*tf.math.log(1e-10+self.weights)) ))** 2 ) #-entropy (trying to maximize entropy)
        self.qs_update = tf.train.AdamOptimizer(self._lr_critic).minimize(self.qs_loss, name='qs_update')
